...
3D 场景 [EN]
3D 对象 [EN]`},{header:"思路",slug:"思路",content:`仓库仿真
通用绘图代码`}]},{path:"/notes/lp.html",title:"模型求解",pathLocale:"/",contents:[{header:"模型求解",slug:"模型求解",content:`在MicroCity中可以对数学模型进行求解。接下来，本文将介绍在MicroCity中求解数学模型的常见过程，并提供一些技巧帮助你更好地建模。
...
local indexList = { GetSTPPath(Network, endID, "iLinkShp") }
上面的这种做法得到的indexList就是一个table类型的变量了。然后就可以开始执行集合的各种操作了捏😋`}]},{path:"/notes/plots.html",title:"通用绘图代码",pathLocale:"/",contents:[{header:"通用绘图代码",slug:"通用绘图代码",content:`本节提供了一些写好的绘图对象，方便需要数据可视化绘图时直接使用。 相关信息
...
以下是用我包装的直方图对象绘制的直方图的示例 如果你已经熟悉直方图对象的用法，点击此处可以 转到代码 对代码进行复制并使用。下面将对直方图对象的用法进行详细介绍。`},{header:"创建对象",slug:"创建对象",content:`创建一个直方图对象
...
属性的设置不是必须的，可以仅根据需要设置。此处就只设置了range、scale、fdisp属性。`},{header:"属性设置",slug:"属性设置",content:`上面提供了直方图属性设置的简单例子。根据代码，还可以设置更多的属性。具体见下表： 属性值
作用
示例 origin
设置图原点（左下角点）
...
fdisp = function(v) return (v/n*100).."%" end，将数据标签改为按照比例显示（假设n为样本数）`},{header:"高级用法",slug:"高级用法",content:"动态刷新数据图 修改图中的数据。此处以向图中添加数据为例。其中假设rnd为添加的数据。也可以直接修改histplot.data的值。 table.insert(histplot.data, rnd) 刷新图 plot:refresh()"},{header:"示例",slug:"示例",content:"示例省去了一些导入的代码。"},{header:"基本用法示例：绘制泊松分布",slug:"基本用法示例-绘制泊松分布",content:`local seed = math.randomseed(1, { distribution = "poisson", mu = "3"
})
...
local seed = math.randomseed(1, { distribution = "poisson", mu = "3"
})
local list = {seed:random()} local histplot = Histogram(list, { range = {0, 10, 0, 50}, scale = {1, 6}
})
scene.render() -- 动态绘制
local count = 1
while count < 1000 do local rnd = seed:random() table.insert(histplot.data, rnd) histplot:refresh() count = count + 1 scene.render()
end`},{header:"子图",slug:"子图",content:"子图(Subplot)是为了解决同时在界面中放置多个图的图定位和尺寸问题。名称借鉴自MATLAB。"},{header:"创建子图对象",slug:"创建子图对象",content:`创建子图时必须输入子图的行数和列数。子图的行数和列数决定了子图的数量。
...
local subplot = Subplot(2, 3, {span = 10}) -- 绘制Subplot对象边界点
...
scene.render()`},{header:"代码",slug:"代码",content:"可以根据代码的组合情况删去开头的 scene.setenv 代码。代码中的 scene.render() 是为了在动态绘制时刷新图像。"},{header:"直方图代码",slug:"直方图代码",content:`Histogram
-- 初始场景设置
...
也就是说，事件调度法主要分为以下3个主要步骤，这3个步骤也是控制事件调度法的主控程序的主要步骤： 时间扫描：确定下—事件发生时间并将仿真时钟推进到该时刻
事件辨识：正确地辨识当前要发生的事件
事件执行：正确地执行当前发生的事件 流程图如下：`},{header:"自动化仓库仿真中使用的简单事件调度法",slug:"自动化仓库仿真中使用的简单事件调度法",content:`自动化仓库仿真思路中使用的是简单的事件调度法，由于当时还没有完整了解事件调度法，因此可能不太成熟，但是也可以作为一种思路参考。主要流程如下: 事先生成任务队列
依次执行任务队列中的任务（事件扫描+事件执行） 由于这个仿真的目的是为了测试仓库的效率，因此在这个仿真中省去了任务相继到达的时间差。所以，每个任务执行完毕后立即执行下一个任务，不需要在任务结束后推进到下一个任务的开始时间（推进量为0）。因此，只需要计量每个任务花费的时间并叠加到总时长（Tick）中。
...
MicroCityWeb version number: Indicates the current version number of MicroCityWeb being used. If there is a difference from the known version, you can force a refresh to load the latest version.`},{header:"Notes",slug:"notes",content:"Turn off automatic translation: If your operating system language is not English and the browser has automatic translation enabled, it needs to be manually disabled, otherwise there will be errors when running the code."}]},{path:"/docs/imgs/",title:"",pathLocale:"/",contents:[{header:"",slug:"",content:"This folder contains images used by documentation pages."}]},{path:"/404.html",title:"",pathLocale:"/",contents:[]}],B="update-vuepress-plugin-full-text-search2-search-index";var C=m(F),W=v(()=>{const e=new Map;for(const t of C.value)e.set(t.path,t);return e});import.meta.webpackHot&&(__VUE_HMR_RUNTIME__[B]=e=>{C.value=e});function V(e){const t=m([]);let s=null;return D(e,()=>{s&&clearTimeout(s),s=setTimeout(a,100)}),t;function a(){const d=e.value.toLowerCase().trim();if(!d){t.value=[];return}const i=new Map,n=new Set;for(const o of C.value)for(const r of U(o,d)){n.add(r.parentPageTitle);let l=i.get(r.parentPageTitle);l||(l=[],i.set(r.parentPageTitle,l)),l.push(r)}const c=[...n].sort((o,r)=>{const l=i.get(o);return i.get(r).length-l.length});t.value=[...i].flatMap(([,o])=>o).sort((o,r)=>o.parentPagePriority-r.parentPagePriority||c.indexOf(o.parentPageTitle)-c.indexOf(r.parentPageTitle)||o.priority-r.priority)}}function*U(e,t){const s=j(e.title,t);if(s){yield{path:e.path,parentPageTitle:w(e),title:e.title,display:s,page:e,content:null,parentPagePriority:1,priority:1};return}for(const a of e.contents){const d=j(a.header,t);if(d){yield{path:e.path+(a.slug?`#${a.slug}`:""),parentPageTitle:w(e),title:e.title,display:d,page:e,content:null,parentPagePriority:10,priority:2};continue}const i=j(a.content,t);i&&(yield{path:e.path+(a.slug?`#${a.slug}`:""),parentPageTitle:w(e),title:e.title,display:[{type:"header",str:`${a.header}
`},...i],page:e,content:null,parentPagePriority:10,priority:10})}}function w(e){const t=e.path.split("/");let s="/";return t[1]&&(s=`/${t[1]}/`),(W.value.get(s)||e).title}function j(e,t){const s=[];let a=0;const d=e.toLowerCase().replace(/\s/gu," ");let i=0,n=d.indexOf(t,i);if(n<0)return null;for(;n>=0;){const o=n+t.length;if(c(e.slice(i,n),"normal"),c(e.slice(n,o),"highlight"),i=o,n=d.indexOf(t,i),a>100)break}return c(e.slice(i),"normal"),s.filter(o=>o.str);function c(o,r){let l=o;r==="normal"&&l.length>100&&a===0&&(l=`… ${l.slice(-10)}`);let g=!1;if(a+l.length>100){if(s.some(b=>b.type==="ellipsis"))return;l=l.slice(0,Math.max(100-a,1)),g=!0}s.push({type:r,str:l}),a+=l.length,g&&(s.push({type:"ellipsis",str:" …"}),a+=2)}}const Y={"/":{placeholder:"搜索"}},X=_({name:"SearchBox",props:{locales:{type:Object,required:!1,default:()=>Y}},setup(e){const{locales:t}=I(e),s=m(""),a=m(!1),d=m(-1),i=V(s),n=v(()=>s.value&&a.value&&i.value.length),c=z(),o=A(),r=v(()=>t.value[o.value]??{});function l(){if(!n.value)return;let h=d.value-1;h<0&&(h=i.value.length-1),b(h)}function g(){if(!n.value)return;let h=d.value+1;h>=i.value.length&&(h=0),b(h)}function b(h){d.value=h}function L(){d.value=-1}function G(h){if(!n.value)return;const k=i.value[h];k&&c.push(k.path)}return{query:s,focused:a,focusIndex:d,suggestions:i,activeSuggestion:n,onUp:l,onDown:g,focus:b,unfocus:L,go:G,locale:r}}});const q={class:"search-box",role:"search"},H=["placeholder"],$=["onMousedown","onMouseenter"],Z=["href"],K={key:0,class:"parent-page-title"},Q={class:"suggestion-row"},J={class:"page-title"},ee={class:"suggestion-content"};function te(e,t,s,a,d,i){return u(),p("div",q,[N(f("input",{ref:"input","onUpdate:modelValue":t[0]||(t[0]=n=>e.query=n),"aria-label":"Search",class:y({focused:e.focused}),placeholder:e.locale.placeholder??"Search",autocomplete:"off",spellcheck:"false",onFocus:t[1]||(t[1]=()=>e.focused=!0),onBlur:t[2]||(t[2]=()=>e.focused=!1),onKeyup:[t[3]||(t[3]=x(n=>e.go(e.focusIndex),["enter"])),t[4]||(t[4]=x((...n)=>e.onUp&&e.onUp(...n),["up"])),t[5]||(t[5]=x((...n)=>e.onDown&&e.onDown(...n),["down"]))]},null,42,H),[[E,e.query]]),e.activeSuggestion?(u(),p("ul",{key:0,class:"suggestions",onMouseleave:t[7]||(t[7]=(...n)=>e.unfocus&&e.unfocus(...n))},[(u(!0),p(T,null,M(e.suggestions,(n,c)=>(u(),p("li",{key:c,class:y(["suggestion",{focused:c===e.focusIndex}]),onMousedown:o=>e.go(c),onMouseenter:o=>e.focus(c)},[f("a",{href:n.path,onClick:t[6]||(t[6]=O(()=>{},["prevent"]))},[n.parentPageTitle&&(!e.suggestions[c-1]||e.suggestions[c-1].parentPageTitle!==n.parentPageTitle)?(u(),p("div",K,S(n.parentPageTitle),1)):P("v-if",!0),f("div",Q,[f("div",J,S(n.title||n.path),1),f("div",ee,[(u(!0),p(T,null,M(n.display,(o,r)=>(u(),p("span",{key:r,class:y(o.type)},S(o.str),3))),128))])])],8,Z)],42,$))),128))],32)):P("v-if",!0)])}const ae=R(X,[["render",te],["__scopeId","data-v-fd6cd4d5"],["__file","SearchBox.vue"]]);export{ae as default};
