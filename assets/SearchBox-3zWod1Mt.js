...
在MicroCityWeb中打开`},{header:"随机数",slug:"随机数",content:""},{header:"随机数生成原理简介",slug:"随机数生成原理简介",content:`在计算机中，随机选择实际上并不是随机的。事实上，我们在计算机中所做的一切都不是随机的，“不可能制造一台真正生成真正随机数的计算机”这件事情是可以被证明的。我们现在获得的随机数都是伪随机数（Pseudo Random）。
...
因此，可以预见的是，这样得到的随机数序列是不可预测的，因为不知道随机数种子会是什么。`},{header:"创建随机数种子",slug:"创建随机数种子",content:`local seed = math.randomseed(x [, dist])
参数 参数
含义 x
