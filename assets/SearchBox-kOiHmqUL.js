...
Lua 调试(Debug) | 菜鸟教程`},{header:"文件保存",slug:"文件保存",content:`MicroCity Web 在保存文件时可以选择保存为 .mw 文件和 .lua 文件。 .mw 文件：打包当前的整个项目，包含当前打开的代码文件和虚拟磁盘中的文件。.mw 文件的压缩格式为 .gz，下载到本地后，如果需要提取文件中的所有文件，可以将 mw 文件的后缀名改为 gz 后再解压查看。
.lua 文件：仅保存当前编辑的代码文件。常用于仅编写 lua 脚本和修改自定义.lua库文件的情况。`},{header:"示例",slug:"示例",content:`假设保存得到 PoissonGraph.mw，重命名为 PoissonGraph.gz 后解压得到如下文件结构
PoissonGraph
├── code
├── Histogram.lua
├── pass
