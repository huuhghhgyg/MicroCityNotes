...
Lua 调试(Debug) | 菜鸟教程`},{header:"文件保存",slug:"文件保存",content:`MicroCity Web 在保存项目文件时会将代码和上传到虚拟磁盘的文件打包保存为 .mw 文件，其压缩格式为 .gz。下载到本地后，如果需要提取文件中的所有文件，可以将 mw 文件的后缀名改为 gz 即可解压。 WIP 🚧
目前发现如果打开单个文件或者编辑完直接保存文件，则保存为 .mw 文件；上传了多个文件的时候，不会保存为 .mw 文件，而是直接将打开的代码保存到本地磁盘打开的源文件中。`},{header:"示例",slug:"示例",content:`假设保存得到 PoissonGraph.mw，重命名为 PoissonGraph.gz 后解压得到如下文件结构
PoissonGraph
├── code
├── Histogram.lua
├── pass
└── Subplot.lua
说明 code: 编辑器中的代码文件。添加后缀名（如重命名为 code.lua）后可以得到编辑器中打开的代码。
Histogram.lua 和 Subplot.lua: 虚拟磁盘中的库文件。`}]},{path:"/notes/event-scheduling.html",title:"离散事件模拟和程序控制",pathLocale:"/",contents:[{header:"离散事件模拟和程序控制",slug:"离散事件模拟和程序控制",content:"本文主要介绍如何在 MicroCity Web 中实现离散事件模拟和程序控制。"},{header:"协程",slug:"协程",content:"MicroCity Web中提供了几个协程相关函数，方便实现事件调度法。如果主要用于实现事件调度法且尚不清楚事件调度法的原理，可以阅读 时间推进法-事件调度法 部分了解相关概念。"},{header:"coroutine.queue",slug:"coroutine-queue",content:`将函数或协程添加到协程队列中等待
...
print('下载完成')`},{header:"导入函数",slug:"导入函数",content:'如果函数文件名为 abc.lua，则引用函数时使用 require("abc") 即可完成导入。'},{header:"函数列表",slug:"函数列表",content:`基本上都是一些自制的函数。
...
If you encounter moiré patterns in the animated GIFs while directly viewing them, you can try opening them to enlarge the view and see if there is any improvement. RMG running process Quay crane running process Overall running process This post is translated using ChatGPT, please feedback if any omissions.`}]},{path:"/en/notes/debug.html",title:"Debugging",pathLocale:"/en/",contents:[{header:"Debugging",slug:"debugging",content:"This page introduces the functions and shortcuts related to debugging in MicroCity Web."},{header:"Clear Output",slug:"clear-output",content:"Right-clicking on the output area allows you to enter commands. Typing print() will clear the contents of the output area."},{header:"Debugger Shortcuts",slug:"debugger-shortcuts",content:`While debugging a program, especially at breakpoints, you can use the following shortcuts to control the program execution: F9 Step Over: Skips the current line and executes the next line.
F10 Step Into: Enters the current line. If the current line is a function call, it steps into the function.
F11 Step Out: Exits the current function and continues the execution of the line after the function call.`},{header:"Debug Commands",slug:"debug-commands",content:""},{header:"Watch Variables",slug:"watch-variables",content:`You can use the debug.watch() command in the console to monitor variables.
For example, if you want to monitor the value of variable a while debugging, you can use the command debug.watch('a'). Right-click on the output area to enter commands.`},{header:"Stack Tracing",slug:"stack-tracing",content:`The debug.traceback() function can be used to print the current stack trace information.
...
[string "function myFunction() ..."]:3: in function 'myFunction'
[string "function myFunction() ..."]:8: in function 'anotherFunction'
[string "function myFunction() ..."]:11: in main chunk`},{header:"Running Mode",slug:"running-mode",content:`debug.debug() enters Debug mode. This is commonly used to enter Debug mode during program execution. If you want to enter Debug mode at startup, you can right-click on the top ▶️ button. See User Interface Introduction - Running Mode for details.
debug.pause() pauses program execution. The top run button ▶️ will light up, and the pause button ⏸️ will turn off.`},{header:"Other Debug Commands",slug:"other-debug-commands",content:`Most of Lua's Debug functions are applicable to MicroCity Web. You can refer to: The Debug Library | Lua 5.4 Reference Manual
Lua Debug | Runoob Tutorial`},{header:"File Saving",slug:"file-saving",content:`When saving project files in MicroCity Web, the code and files uploaded to the virtual disk are saved as .mw files with a compression format of .gz. After downloading to the local computer, if you need to extract all the files from the file, you can change the file extension of the mw file to gz and then decompress it. WIP 🚧
Currently, it has been found that if you open a single file or save the file directly after editing, it will be saved as an .mw file. If you upload multiple files, they will not be saved as .mw files, but the opened code will be saved directly to the source file opened on the local disk.`},{header:"Example",slug:"example",content:`Assuming that you saved a file called PoissonGraph.mw, you can rename it to PoissonGraph.gz and decompress it to obtain the following file structure:
PoissonGraph
├── code
├── Histogram.lua
├── pass
└── Subplot.lua
Description code: The code file in the editor. After a suffix is added (rename it as code.lua for example), the code opened in the editor can be obtained.
Histogram.lua and Subplot.lua: The library files in the virtual disk. This post is translated using ChatGPT, please feedback if any omissions.`}]},{path:"/en/notes/event-scheduling.html",title:"Discrete Event Simulation and Program Control",pathLocale:"/en/",contents:[{header:"Discrete Event Simulation and Program Control",slug:"discrete-event-simulation-and-program-control",content:""},{header:"Coroutines",slug:"coroutines",content:"MicroCityWeb provides several coroutine-related functions that facilitate the implementation of event scheduling."},{header:"coroutine.queue",slug:"coroutine-queue",content:`Adds a function or coroutine to the coroutine queue for execution after a specified relative time.
...
Sometimes there may not be much time-consuming operation between two recorded times, and the calculated value of dt may be 0. Generally, there is a scene.render() between two time recordings, which usually ensures that a non-zero time difference can be calculated between the two samples. This post is translated using ChatGPT, please feedback if any omissions.`}]},{path:"/en/notes/tools.html",title:"Tools",pathLocale:"/en/",contents:[{header:"Tools",slug:"tools",content:"Possible tools that may be used."},{header:"Third-party Tools",slug:"third-party-tools",content:`Includes software, websites, etc. MapShaper Map editor. It can edit .geojson files and export them as .shp files.
Three.js Editor Online 3D object editor for Three.js.`},{header:"External Functions",slug:"external-functions",content:"Some external functions for MicroCity Web."},{header:"How to Use",slug:"how-to-use",content:`It is recommended to import functions online to ensure that the latest version is used. You can also choose to download them locally and then upload for use.
...
print('Download completed')`},{header:"Importing Functions",slug:"importing-functions",content:'If the function file name is abc.lua, you can import the function by using require("abc").'},{header:"Function List",slug:"function-list",content:`Mostly homemade functions.
...
`},...a],page:e,content:null,parentPagePriority:10,priority:10})}}function v(e){const t=e.path.split("/");let s="/";return t[1]&&(s=`/${t[1]}/`),(q.value.get(s)||e).title}function S(e,t){const s=[];let i=0;const c=e.toLowerCase().replace(/\s/gu," ");let a=0,n=c.indexOf(t,a);if(n<0)return null;for(;n>=0;){const o=n+t.length;if(d(e.slice(a,n),"normal"),d(e.slice(n,o),"highlight"),a=o,n=c.indexOf(t,a),i>100)break}return d(e.slice(a),"normal"),s.filter(o=>o.str);function d(o,r){let l=o;r==="normal"&&l.length>100&&i===0&&(l=`… ${l.slice(-10)}`);let m=!1;if(i+l.length>100){if(s.some(g=>g.type==="ellipsis"))return;l=l.slice(0,Math.max(100-i,1)),m=!0}s.push({type:r,str:l}),i+=l.length,m&&(s.push({type:"ellipsis",str:" …"}),i+=2)}}var H={"/":{placeholder:"搜索"},"/en/":{placeholder:"Search"}};const U=H,Y=I({name:"SearchBox",props:{locales:{type:Object,required:!1,default:()=>U}},setup(e){const{locales:t}=D(e),s=b(""),i=b(!1),c=b(-1),a=W(s),n=j(()=>s.value&&i.value&&a.value.length),d=_(),o=O(),r=j(()=>t.value[o.value]??{});function l(){if(!n.value)return;let h=c.value-1;h<0&&(h=a.value.length-1),g(h)}function m(){if(!n.value)return;let h=c.value+1;h>=a.value.length&&(h=0),g(h)}function g(h){c.value=h}function P(){c.value=-1}function A(h){if(!n.value)return;const k=a.value[h];k&&d.push(k.path)}return{query:s,focused:i,focusIndex:c,suggestions:a,activeSuggestion:n,onUp:l,onDown:m,focus:g,unfocus:P,go:A,locale:r}}}),X={class:"search-box",role:"search"},$=["placeholder"],Z=["onMousedown","onMouseenter"],Q=["href"],J={key:0,class:"parent-page-title"},K={class:"suggestion-row"},ee={class:"page-title"},te={class:"suggestion-content"};function ne(e,t,s,i,c,a){return u(),p("div",X,[R(f("input",{ref:"input","onUpdate:modelValue":t[0]||(t[0]=n=>e.query=n),"aria-label":"Search",class:y({focused:e.focused}),placeholder:e.locale.placeholder??"Search",autocomplete:"off",spellcheck:"false",onFocus:t[1]||(t[1]=()=>e.focused=!0),onBlur:t[2]||(t[2]=()=>e.focused=!1),onKeyup:[t[3]||(t[3]=x(n=>e.go(e.focusIndex),["enter"])),t[4]||(t[4]=x((...n)=>e.onUp&&e.onUp(...n),["up"])),t[5]||(t[5]=x((...n)=>e.onDown&&e.onDown(...n),["down"]))]},null,42,$),[[N,e.query]]),e.activeSuggestion?(u(),p("ul",{key:0,class:"suggestions",onMouseleave:t[7]||(t[7]=(...n)=>e.unfocus&&e.unfocus(...n))},[(u(!0),p(T,null,M(e.suggestions,(n,d)=>(u(),p("li",{key:d,class:y(["suggestion",{focused:d===e.focusIndex}]),onMousedown:o=>e.go(d),onMouseenter:o=>e.focus(d)},[f("a",{href:n.path,onClick:t[6]||(t[6]=E(()=>{},["prevent"]))},[n.parentPageTitle&&(!e.suggestions[d-1]||e.suggestions[d-1].parentPageTitle!==n.parentPageTitle)?(u(),p("div",J,w(n.parentPageTitle),1)):G("v-if",!0),f("div",K,[f("div",ee,w(n.title||n.path),1),f("div",te,[(u(!0),p(T,null,M(n.display,(o,r)=>(u(),p("span",{key:r,class:y(o.type)},w(o.str),3))),128))])])],8,Q)],42,Z))),128))],32)):G("v-if",!0)])}const ie=z(Y,[["render",ne],["__scopeId","data-v-fd6cd4d5"],["__file","SearchBox.vue"]]);export{ie as default};
