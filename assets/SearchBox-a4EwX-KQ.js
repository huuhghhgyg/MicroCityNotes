...
设置一个物体的id。
GetObject (Scene, "id")
根据id查找一个物体。 这篇文章使用ChatGPT翻译自其他语言，如果有问题请在反馈页面提交反馈。`}]},{path:"/docs/4.6_networks.html",title:"4.6 网络",pathLocale:"/",contents:[{header:"4.6 网络",slug:"_4-6-网络",content:'在MicroCity中，GIS矢量数据（参见3.2）可以进行拓扑操作，并应用最短路径算法和用户均衡交通分配算法。本章的示例代码可以在项目选项卡的"network_samples"文件夹中找到，具体请参考ScriptEditor。'},{header:"网络操作",slug:"网络操作",content:`CreateNetwork ()
创建一个空网络对象并返回。
...
从随机数生成引擎中获取一个随机数。`},{header:"模拟控制",slug:"模拟控制",content:`CreateEvent (time, Funcion [, arg1, arg2, ...])
...
将LPModel写入文件(*.lp, *.mps)，并在成功时返回true。 这篇文章使用ChatGPT翻译自其他语言，如果有问题请在反馈页面提交反馈。`}]},{path:"/docs/",title:"MicroCity Desktop 文档",pathLocale:"/",contents:[{header:"MicroCity Desktop 文档",slug:"microcity-desktop-文档",content:""},{header:"目录",slug:"目录",content:`1 Introduction 1.1 MicroCity的功能 2 入门指南 2.1 显示世界地图
...
4.6 网络
4.7 离散事件仿真
4.8 混合整数规划 声明：本网站文档中的部分内容来源于MicroCity Desktop，版权归原作者所有。 这篇文章使用ChatGPT翻译自其他语言，如果有问题请在反馈页面提交反馈。`}]},{path:"/notes/3d-objects.html",title:"3D 对象",pathLocale:"/",contents:[{header:"3D 对象",slug:"_3d-对象",content:"本节将介绍 MicroCity Web 中的三维场景对象及其控制函数。"},{header:"添加对象",slug:"添加对象",content:`向场景中添加对象，函数形式如下
...
在MicroCityWeb中打开`},{header:"随机数",slug:"随机数",content:""},{header:"创建随机数种子",slug:"创建随机数种子",content:`local seed = math.randomseed(x [, dist])
参数 参数
含义 x
...
end`}]},{path:"/notes/timelapse.html",title:"仿真时间推进",pathLocale:"/",contents:[{header:"仿真时间推进",slug:"仿真时间推进",content:"仿真中的时间推进是指在仿真过程中，通过什么方法推进仿真时间。"},{header:"时间推进法的分类",slug:"时间推进法的分类",content:`常见的仿真时间推进方法有三大类： 事件调度法：事件调度法是一种基于事件的仿真方法，它通过对仿真系统中各个事件进行排序，按照事件发生的先后顺序依次推进时间执行活动，进而实现仿真。
...
有时两次记录时间之间没有太多耗时的操作，计算得到的 dt 可能为0。一般两次时间记录之间存在一个 scene.render()，这样一般能够保证两次采样的时间之间能够计算得到一个不为0的时间差值。`}]},{path:"/notes/tools.html",title:"有关工具",pathLocale:"/",contents:[{header:"有关工具",slug:"有关工具",content:`可能能够用到的工具 MapShaper 地图编辑器。可以编辑.geojson文件并导出为.shp文件
Three.js Editor Three.js的在线3D对象编辑器。 🔨 持续更新...`}]},{path:"/notes/versions.html",title:"MicroCity的版本",pathLocale:"/",contents:[{header:"MicroCity的版本",slug:"microcity的版本",content:""},{header:"MicroCity",slug:"microcity",content:"MicroCity 软件最初推出时只有一个桌面版本，只能在 Windows 操作系统上运行。随着时间的推移，开发者（@mixwind)意识到这个限制使得许多潜在的用户无法使用该软件，因为他们的电脑使用的可能是其他操作系统，比如MacOS、Linux等。"},{header:"MicroCityWeb",slug:"microcityweb",content:`🎯访问MicroCity Web 📍(离线版)下载
...
由于使用了实验性的 JavaScript函数，MicroCity Web 可能在以下浏览器内核中运行存在兼容性问题： iOS和iPad OS上的大多数浏览器（由于Apple强制使用Safari的浏览器内核）
FireFox浏览器 2023/9/29测试遇到的兼容性问题及部分技术细节 iOS 和 iPad OS： 无法打开本地文件（由于使用了 showOpenFilePicker() 方法） FireFox： 无法打开本地文件（由于使用了 showOpenFilePicker() 方法） 关于 showOpenFilePicker() 方法的影响参见 MDN Web Docs | showOpenFilePicker()`},{header:"总结",slug:"总结",content:`MicroCity Web 版的推出使得软件更加易用和普及，让更多的用户可以从中受益。同时，MicroCity 桌面版本仍然可用，使得那些已经习惯于使用该版本的用户可以继续使用，并让他们更容易地过渡到 MicroCity Web 版。
本文作者在写这篇笔记的时候 MicroCity Web 正在被第一次用于教学环境中，已经基本能够正常运行并实现一定的仿真功能。本文作者的学习经历如下： MicroCity（桌面版）：物流信息管理
...
如果您在弹出的对话框中输入了密码，那么在其他人打开链接后，他们只能运行您的程序代码并观察运行结果，而无法查看您的代码（除非您告知他们密码）；如果您没有输入密码，则其他人不仅可以运行您的代码查看运行结果，还可以查看您编写的代码。 查看帮助文档。`},{header:"绿色：代码编辑器",slug:"绿色-代码编辑器",content:"MicroCity Web左侧整合的代码编辑器源于AceEditor，因此也具有AceEditor的一些特性。"},{header:"快捷键",slug:"快捷键",content:`下面将介绍一些常用的快捷键，帮助你更好地编辑代码。
...
对象旋转：点击对象，对象颜色改变后从对象上按住鼠标左键拖动，以对象为中心旋转。 无论是二维对象还是三维对象都遵循上述操作逻辑。`},{header:"黄色：代码输出区域",slug:"黄色-代码输出区域",content:`打开MicroCity Web后，代码输出区域如下。点击代码输出区域后整个区域会上移，能够看到更多的历史输出信息。 说到这个区域输出的信息，代码输出区域会输出以下信息： 脚本输出: 左侧是脚本的输出，包含输出时间、脚本输出内容。
...
I believe that the following diagram from the course can effectively represent how the event scheduling method is used for simulation and scene refreshing. The first row refers to the business process of simulation, and the Refresh part in the second row refers to refreshing the scene.`},{header:"Results",slug:"results-1",content:`Note
...
If you encounter moiré patterns in the animated GIFs while directly viewing them, you can try opening them to enlarge the view and see if there is any improvement. RMG running process Quay crane running process Overall running process This post is translated using ChatGPT, please feedback if any omissions.`}]},{path:"/en/notes/debug.html",title:"Debugging",pathLocale:"/en/",contents:[{header:"Debugging",slug:"debugging",content:"This page introduces the functions and shortcuts related to debugging in MicroCity Web."},{header:"Clearing Output",slug:"clearing-output",content:"Right-clicking on the output area allows you to enter commands. Entering print() clears the content of the output area."},{header:"Debugger Shortcuts",slug:"debugger-shortcuts",content:`During the process of debugging a program, especially at breakpoints, you can use F9 (Step Over), F10 (Step Into), and F11 (Step Out) to control the program execution. F9 Step Over: Skips the current line and executes the next line.
F10 Step Into: Enters the current line. If the current line is a function call, it enters the function body.
F11 Step Out: Exits the current function and executes the next line after the function call.`},{header:"Debug Commands",slug:"debug-commands",content:""},{header:"Watching Variables",slug:"watching-variables",content:`Use the debug.watch() command in the console to monitor variables.
For example, if you want to monitor the value of variable a during debugging, you can use the debug.watch('a') command. Right-clicking on the output area allows you to enter commands.`},{header:"Stack Trace",slug:"stack-trace",content:`The debug.traceback() function can print the current stack trace information.
...
[string "function myFunction() ..."]:3: in function 'myFunction'
[string "function myFunction() ..."]:8: in function 'anotherFunction'
[string "function myFunction() ..."]:11: in main chunk`},{header:"Running Status",slug:"running-status",content:`debug.debug() enters Debug mode. It is often used to enter Debug mode during program execution. If you want to start in Debug mode, you can right-click the top ▶️ button. See User Interface Introduction - Running Status for details.
debug.pause() pauses program execution. The Run button ▶️ at the top will light up, and the Pause button ⏸️ will turn off.`},{header:"Other Debug Commands",slug:"other-debug-commands",content:`Most of Lua's Debug functions are applicable to MicroCity Web. For reference, please see: The Debug Library | Lua 5.4 Reference Manual
...
function update() if not scene.render() then return end -- Render the scene and check if the program is terminated coroutine.queue(dt, update) -- Add the next update based on CPU step time carmove() -- Move the vehicle refreshtime() -- Calculate the current dt
...
local plot = Histogram(list, {range={40,90,0,100}, scale={10, 50}, fdisp=function(x) return (x/n*100).."%" end})
Setting the properties is not necessary and can be done as needed. In this case, only the range, scale, and fdisp properties are set.`},{header:"Setting Histogram Properties",slug:"setting-histogram-properties",content:`The above example provides a simple demonstration of setting histogram properties. According to the code, more properties can be set. See the table below for details: Property
Function
Example origin
Set the origin of the graph (bottom-left point)
origin = {-50, -50}, set the origin of the graph at position (-50,-50) size
Set the length of the x and y axes of the graph
size = {100, 100}, set the length of the x and y axes of the graph to 100 each scale
Set the graduations of the x and y axes
scale = {3, 2}, set the graduations of the x and y axes of the graph to 3 and 2 respectively axislabel
Set the titles of the x and y axes
axislabel = {"x", "y"}, set the titles of the x and y axes of the graph to "x" and "y" respectively range
Set the display range of the x and y axes
range = {0, 10, 0, 20}, set the x-axis range of the graph to [0,10] and the y-axis range to [0,20] filled
Set whether to fill the bar chart
filled = false, set the bar chart to unfilled. The default style is filled fdisp
The display function for data labels, which defaults to the quantity. When setting the display function for data labels, it will also affect the display of y-axis tick marks.
fdisp = function(v) return (v/n*100).."%" end, change the data labels to display in proportion (assuming n is the sample size)`},{header:"Advanced Usage of Histograms",slug:"advanced-usage-of-histograms",content:"Dynamically refresh the data plot. Modify the data in the graph. Here, we use adding data to the graph as an example. Let's assume rnd is the data to be added. You can also directly modify the value of plot.data. table.insert(plot.data, rnd) Refresh the graph. plot:refresh()"},{header:"Example",slug:"example",content:""},{header:"Basic Usage Example: Drawing a Poisson Distribution",slug:"basic-usage-example-drawing-a-poisson-distribution",content:`require("Histogram") --Reference the histogram local seed = math.randomseed(1, { distribution = "poisson", mu = "3"
})
local list = {seed:random()} --Static drawing
for i = #list, 400 do table.insert(list, seed:random())
end local histplot = Histogram(list, { range = {0, 10, 0, 50}, scale = {1, 5}
})
scene.render()`},{header:"Advanced Usage Example: Dynamically Refreshing Plot of the Poisson Distribution",slug:"advanced-usage-example-dynamically-refreshing-plot-of-the-poisson-distribution",content:`This is an example of plotting the Poisson distribution and dynamically refreshing the data plot. It is assumed that the Histogram object has been imported and the scene is set for 2D display.
require("Histogram") -- Import the histogram local seed = math.randomseed(1, { distribution = "poisson", mu = "3"
})
local list = {seed:random()} local histplot = Histogram(list, { range = {0, 10, 0, 50}, scale = {1, 8}
})
scene.render() -- Dynamic drawing
local count = 1
while count < 1000 do local rnd = seed:random() table.insert(histplot.data, rnd) histplot:refresh() count = count + 1 scene.render()
end`},{header:"Subplots",slug:"subplots",content:"Subplots are a way to position and size multiple plots within a single interface. The name is borrowed from MATLAB."},{header:"Importing Subplots",slug:"importing-subplots",content:'First, import the code for subplots. Like histograms, there are two options for importing: Download the code file and import (recommended): Download the subplot code file, import the Subplot.lua file in MicroCityWeb, and then use the code. Use the following code to import:require("Subplot") -- Import the subplot code Copy the code: Copy the code from the Subplot.lua file directly into your code. In this case, there is no need to import the code. See the code below.'},{header:"Creating Subplot Objects",slug:"creating-subplot-objects",content:`When creating a subplot, you must specify the number of rows and columns. The number of rows and columns determines the number of subplots.
local subplot = Subplot(rows, cols)
...
local subplot = Subplot(rows, cols, {span=10, diag={-80, -70, 80, 70}})`},{header:"Setting Subplot Properties",slug:"setting-subplot-properties",content:`Here, we will provide a detailed description of the properties of subplots. Setting these properties is optional and can be done as needed. Property
Description
Example span
Sets the spacing between subplots
span=10, sets the spacing between subplots to 10 in all directions diag
Sets the position and size of a subplot by specifying the coordinates of the two diagonal points
...
Here, it is assumed that the subplot object is stored in the subplot variable after creation. Attribute
Meaning
Example originpt
The origin position of each subplot in the Subplot object
The origin position of the subplot in the i-th row and j-th column can be obtained by subplot.originpt[i][j][1] and subplot.originpt[i][j][2] psize
The size of each subplot in the Subplot object
The width and height of the subplot in the subplot can be obtained by subplot.psize[1] and subplot.psize[2].(assuming all subplots have the same size)`},{header:"Example",slug:"example-1",content:`This is an example of drawing the range of subplots. Assume that the Subplot object has been imported and the scene has been set to 2D display.
...
require("Subplot") -- import the Subplot library local subplot = Subplot(2, 3, {span = 10}) -- Draw the boundary points of the Subplot object
local lb = scene.addobj("points",{size = 10, vertices={subplot.diag[1],subplot.diag[2],0}})
local lt = scene.addobj("points",{size = 10, vertices={subplot.diag[1],subplot.diag[4],0}})
local rb = scene.addobj("points",{size = 10, vertices={subplot.diag[3],subplot.diag[4],0}})
local rt = scene.addobj("points",{size = 10, vertices={subplot.diag[3],subplot.diag[2],0}}) -- Draw the range of each subplot
for i = 1, subplot.row do for j = 1, subplot.col do scene.addobj("polyline", {size = 8, color = "blue", vertices={ subplot.originpt[i][j][1]+subplot.psize[1], subplot.originpt[i][j][2], 0, subplot.originpt[i][j][1], subplot.originpt[i][j][2], 0, subplot.originpt[i][j][1], subplot.originpt[i][j][2]+subplot.psize[2], 0, subplot.originpt[i][j][1]+subplot.psize[1], subplot.originpt[i][j][2]+subplot.psize[2], 0, subplot.originpt[i][j][1]+subplot.psize[1], subplot.originpt[i][j][2], 0, } }) end
end
scene.render()`},{header:"Code",slug:"code",content:"You can remove the scene.setenv code at the beginning depending on the combination of your code. The scene.render() in the code is used to refresh the image during dynamic drawing."},{header:"Code File Download",slug:"code-file-download",content:`A list of code files can be found in ModelResource/libs Name
Link Histogram
ModelResource/libs/Histogram.lua Subplot
ModelResource/libs/Subplot.lua`},{header:"Histogram Code",slug:"histogram-code",content:`Histogram
-- Initial scene settings
local s = scene.setenv({camtype = "ortho"}) -- Returns a histogram object
function Histogram(data, ...) local plot = { -- Default value settings origin = {-50, -50}, -- Coordinate origin coordinates size = {100, 100}, -- Coordinate axis length scale = {3, 2}, -- Coordinate axis scale values range = {0, 10, 0, 20}, -- xy-axis range axislabel = {"x", "y"}, filled = true, -- Whether to fill fdisp = function(v) -- Data label display return v end, } -- Import parameters and fallback if ... ~= nil then for k, v in pairs(...) do plot[k] = v end end local barShape = "polygon" if plot.filled == false then barShape = "polyline" end -- Initialization plot.axes = {} -- Coordinate axes plot.originlabel = {} -- The zero point may have dual coordinates plot.bars = {} -- Data bars plot.xvalue = {} -- x-axis tick values (excluding the minimum value) plot.data = data function plot:deldata() -- Delete data (private) if #plot.bars == 0 then return end for i = #plot.bars, 1, -1 do -- print("plot.bar:",i) plot.bars[i][2]:delete() plot.bars[i][1]:delete() end plot.zero:delete() for i = #plot.originlabel, 1, -1 do plot.originlabel[i]:delete() end for i = #plot.axes, 1, -1 do for j = #plot.axes[i], 1, -1 do plot.axes[i][j][2]:delete() plot.axes[i][j][1]:delete() end end end function plot:refresh() -- Called when drawing repeatedly (public) plot:deldata()
...
plot.axes[1] = xaxis for i = 1, #plot.axes[1] do -- based on x-axis score local ub = plot.xvalue[i] -- current loop maximum value -- print("Loop", i, "ub=", ub) -- for debugging local value = 0 for j = #barData, 1, -1 do if barData[j] <= ub then value = value + 1 table.remove(barData, j) end end plot.barValue[i] = value -- record value end local ymax = math.max(table.unpack(plot.barValue)) plot.range[4] = math.max(ymax, plot.range[4]) -- Y-axis scale
local yaxis = {} -- y-axis objects, each element is {point, label}
for i = 1, (plot.range[4] - plot.range[3]) / plot.scale[2] do -- index of each scale local ypt = plot.origin[2] + i * plot.size[2] / (plot.range[4] - plot.range[3]) * plot.scale[2] -- Y-scale position local pt = scene.addobj("points", { vertices = {plot.origin[1], ypt, 0}, size = 5 }) -- scale point local label = scene.addobj("label", { text = plot.fdisp(plot.range[3] + i * plot.scale[2]), size = 4 }) -- scale label label:setpos(plot.origin[1] - 4, ypt, 0) yaxis[i] = {pt, label}
end
plot.axes[2] = yaxis -- Draw data
local xunit = plot.size[1] / (plot.range[2] - plot.range[1]) * #plot.axes[1] -- x-axis division value
local yunit = plot.size[2] / (plot.range[4] - plot.range[3]) -- y-axis division value -- Draw histogram based on the values
for i = 1, #plot.axes[1] do -- Drawing local xl = plot.origin[1] + (i - 1) * plot.size[1] / (plot.range[2] - plot.range[1]) * plot.scale[1] local xr = plot.origin[1] + i * plot.size[1] / (plot.range[2] - plot.range[1]) * plot.scale[1] local yb, yt = plot.origin[2], plot.origin[2] + plot.barValue[i] * yunit local bar = scene.addobj(barShape, { vertices = {xl, yb, 0, xr, yb, 0, xr, yt, 0, xl, yt, 0, xl, yb, 0} }) local label = scene.addobj("label", { text = plot.fdisp(plot.barValue[i]), size = 4 }) label:setpos((xl + xr) / 2, yt + 2, 0) plot.bars[i] = {bar, label}
end
function plot:draw() -- Draw initialization -- Coordinate axis object plot.coord = scene.addobj("polyline", { vertices = {plot.origin[1] + plot.size[1], plot.origin[2], 0, plot.origin[1], plot.origin[2], 0, plot.origin[1], plot.origin[2] + plot.size[2], 0} }) -- Coordinate axis title if plot.axislabel~=nil then plot.axislabel[1] = scene.addobj("label", { text = plot.axislabel[1], size = 4 }) plot.axislabel[1]:setpos(plot.origin[1] + plot.size[1], plot.origin[2] - 8, 0) plot.axislabel[2] = scene.addobj("label", { text = plot.axislabel[2], size = 4, }) plot.axislabel[2]:setpos(plot.origin[1]-4, plot.origin[2] + plot.size[2]+6, 0) plot.axislabel[2]:setrot(0,0,1) end
end plot:draw()
plot:refresh()
scene.render()
return plot
end`},{header:"Subplot code",slug:"subplot-code",content:`Subplot
-- Attribute parameters
-- local diag = {-80, -70, 80, 70} -- lbx,lby,rtx,rty
-- local span = 10
-- local row, col = 2, 2
function Subplot(row, col, ...) local subplot = { span = 10, diag = {-80, -70, 80, 70}, originpt = {}, -- Store coordinates of each row and column psize = {} -- Subplot size } subplot.row, subplot.col = row, col \`\`\`lua
-- Import parameters and fallback
if ... ~= nil then for k, v in pairs(...) do subplot[k] = v end
end function subplot:refresh() -- Calculate parameters subplot.psize = { (subplot.diag[3]-subplot.diag[1] - (subplot.col-1)*subplot.span)/subplot.col, (subplot.diag[4]-subplot.diag[2] - (subplot.row-1)*subplot.span)/subplot.row } local gwidth, gheight = subplot.psize[1]+subplot.span, subplot.psize[2]+subplot.span -- Recalculate originpt subplot.originpt = {} for i = 1, subplot.row do subplot.originpt[i]={} for j = 1, subplot.col do subplot.originpt[i][j] = {subplot.diag[1]+(j-1)*gwidth, subplot.diag[2]+(i-1)*gheight} end end
end subplot:refresh()
return subplot
...
Master clock advancement method: The master clock advancement method refers to controlling the entire simulation system during the simulation process using a master clock. It controls the simulation process by controlling the clock.`},{header:"Example",slug:"example",content:""},{header:"Event Scheduling Method",slug:"event-scheduling-method",content:`The event scheduling method is an event-based simulation method that sorts events in the simulation system and simulates them based on the order of their occurrence. During the simulation process, the simulation system determines the next event to occur based on the current simulation time and the time of each event, and advances the simulation time to the time when the event occurs.
The event scheduling method consists of the following three main steps, which are also the main steps of the control program for the event scheduling method: Time scanning: Determine the time when the next event will occur and advance the simulation clock to that moment.
Event identification: Correctly identify the event that is about to occur.
Event execution: Correctly execute the event that has occurred. The basic principle flowchart of the event scheduling method is as follows:
...
local x = 1
local y = 1
local z = 0 local rx, ry = 0.1, 0.1 -- Rotation speed in the x and y directions
...
Explanation: When the simulation speed simspeed is 1, it means that the simulation clock is moving at the same speed as the real-world clock. simspeed can be considered as an acceleration factor.
...
Info
It should be noted that the value of the time increment dt is generally not the same in each loop cycle, and the specific value of dt generally depends on the computational power of the computer. Tips
Sometimes there is not much time-consuming operation between two recorded times, and the calculated dt may be 0. Generally, there is a scene.render() between two time recordings, which usually ensures a non-zero time difference between the two samples. This post is translated using ChatGPT, please feedback if any omissions.`}]},{path:"/en/notes/tools.html",title:"Tools",pathLocale:"/en/",contents:[{header:"Tools",slug:"tools",content:`Possible tools that may come in handy: MapShaper - a map editor tool. It allows you to edit .geojson files and export them as .shp files.
Three.js Editor - an online 3D object editor for Three.js. 🔨 Work In Progress... This post is translated using ChatGPT, please feedback if any omissions.`}]},{path:"/en/notes/versions.html",title:"Versions of MicroCity",pathLocale:"/en/",contents:[{header:"Versions of MicroCity",slug:"versions-of-microcity",content:""},{header:"MicroCity",slug:"microcity",content:"When MicroCity software was initially launched, there was only a desktop version available, which could only run on Windows operating systems. Over time, the developer (@mixwind) realized that this limitation prevented many potential users from using the software because they might be using other operating systems such as MacOS, Linux, and others."},{header:"MicroCityWeb",slug:"microcityweb",content:`🎯Visit MicroCity Web 📍(Offline version) Download
