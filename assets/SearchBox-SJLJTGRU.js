import{f as b,g as j,h as L,i as _,t as I,u as D,j as O,_ as z,o as u,c as p,k as N,v as R,a as f,n as y,l as x,F as T,m as M,p as G,q as w,s as E}from"./app-Gl7DLcIg.js";const F=[{path:"/",title:"首页",pathLocale:"/",contents:[{header:"开始使用MicroCity Web",slug:"开始使用microcity-web",content:`打开MicroCity Web，默认加载方块旋转的示例代码。 点击运行按钮运行代码 程序运行，看到右侧视图中方块旋转。 更多MicroCity的使用方法可以参考本站翻译的 MicroCity Web中文文档。
...
scene.render() --渲染场景`},{header:"展示结果",slug:"展示结果",content:"运行脚本将会突出显示搜索到的国家，并在下方的消息栏中打印信息。您可以点击消息栏以放大它。 < 2.1 掷骰子 | 目录 | 3.1 用户界面概览 > 本文使用ChatGPT翻译，如有遗漏请反馈。"}]},{path:"/docs/web/3.1_ui_overview.html",title:"3.1 用户界面概览",pathLocale:"/",contents:[{header:"3.1 用户界面概览",slug:"_3-1-用户界面概览",content:"MicroCity Web 提供了一个紧凑而多功能的用户界面。"},{header:"界面划分及功能",slug:"界面划分及功能",content:"MicroCity Web 的用户界面分为四个部分：按钮面板，脚本编辑器，场景画布和消息栏，如下图所示： 这里我们简要介绍按钮面板，脚本编辑器，场景画布和消息栏将在后续章节中详细讨论：3.2 编辑脚本，3.3 运行脚本和3.4 导航场景。"},{header:"按钮面板",slug:"按钮面板",content:`按钮面板包含一个徽标链接（单击以打开新窗口），九个可单击的按钮，其中大多数还可以右键单击。对话框，文档和脚本编辑器可以通过 ESC 按钮关闭。以下是每个按钮的功能表格。 按钮
名称
快捷键
左键点击
右键点击 运行/调试
...
Ctrl + S
保存脚本/项目文件
从虚拟磁盘下载文件 发布/分享 将项目发布到网络
分享文件到网络 文档
F1
显示文档`},{header:"文件管理",slug:"文件管理",content:`MicroCity Web可以将脚本和虚拟磁盘文件打包成单个 .wm 文件，方便携带。您还可以使用 发布 将其保存为一个 URL 链接供公开访问。以下表格显示了 MicroCity Web 支持的文件类型。 类型
格式
用途
备注 .wm
MicroCity Web 文件
...
位图图像
scene.setmat
作为场景对象的纹理使用 此外，通过右键单击 打开、直接拖放或使用 上传 功能，可以上传任何类型的文件到虚拟磁盘，以便脚本能够直接访问它（示例脚本中的资源位于 '/res' 目录中）。通过右键单击 保存 或使用 下载 功能，也可以将虚拟磁盘中的文件下载到本地。您还可以通过右键单击 发布 将任何文件保存为 URL 链接。 本文使用ChatGPT翻译，如有遗漏请反馈。`}]},{path:"/docs/web/3.2_editing_scripts.html",title:"3.2 编辑脚本",pathLocale:"/",contents:[{header:"3.2 编辑脚本",slug:"_3-2-编辑脚本",content:"MicroCity 网页应用程序采用Lua脚本编写。除了熟悉Lua语法外，用户还需要熟练使用脚本编辑器。"},{header:"脚本编辑器",slug:"脚本编辑器",content:"集成的脚本编辑器基于Ace，易于使用且功能丰富。下图显示了编辑器窗口中经常出现的图形和符号。在调试脚本时会使用“断点”和“当前行”。"},{header:"快捷键",slug:"快捷键",content:`大多数快捷键遵循Ace的设置，并且一些是由MicroCity Web自定义的。以下是最常用的快捷键列表： Ctrl + B 新建文件
Ctrl + S 保存文件
Ctrl + O 打开文件
Ctrl + Z 撤销
Ctrl + Y 重做
...
步进 debug.stepo
F11
步出 debug.watch 监视变量 本文使用ChatGPT翻译，如有遗漏请反馈。`}]},{path:"/docs/web/3.4_navigating_scenes.html",title:"3.4 浏览场景",pathLocale:"/",contents:[{header:"3.4 浏览场景",slug:"_3-4-浏览场景",content:"脚本加载或生成的 3D/2D 对象可以显示在场景画布上，用户可以通过导航手势从不同角度查看它们。"},{header:"场景画布",slug:"场景画布",content:"您可以创建一个新的脚本，从“加载 GLTF”示例中复制并运行它。场景画布可以展示一个3D虚拟世界。坐标轴xyz之间的关系满足右手定则（如图所示），摄像机的初始位置是 (20, 10, 20)。当场景摄像机设置为z轴正交投影（camtype='ortho'）时，在场景画布中显示的3D世界将变为'2D'，摄像机的初始位置为 (0, 0, 20)。对象可以被选择，选择将在脚本中进行处理。其他操作，如平移、旋转和缩放，也可以通过脚本完成。旋转遵循右手定则。详情请参阅 4.3 场景和对象。"},{header:"导航手势",slug:"导航手势",content:"类似于 2.1 滚动立方体，这里我们呈现另一个更易于理解和练习的示例，如下图所示："},{header:"外部工具",slug:"外部工具",content:`为了更轻松地编辑和预览对象，以下是一些有用的工具： GeoJSON文件编辑器：https://geojson.io
...
此功能用于使脚本访问外部文件变得更便捷。用户可以先上传文件，然后启动程序。 os.download ([fpath]) 从fpath路径或提示用户选择要从虚拟磁盘下载的文件。`},{header:"大语言模型",slug:"大语言模型",content:`这些函数可以触发用于对话完成和嵌入的远程REST API。API可以进行定制。 os.chatcmpl (system, user)
给定系统消息（提示）和用户消息组成的会话，模型将返回响应。 os.embedding (input)
从fpath路径访问文件或提示用户选择要从虚拟磁盘下载的文件。`},{header:"大语言模型",slug:"大语言模型-1",content:`这些函数可以触发用于对话完成和嵌入的远程REST API。API可以进行定制。 os.chatcmpl (system, user, [maxtokens])
给定系统消息（提示）和用户消息组成的会话，模型将返回响应。 os.embedding (input)`},{header:"调试",slug:"调试",content:`调试库保留了 Lua 的原始功能并进行了修改。用户可以在脚本中或控制台上执行它们。 debug.debug ([b]) 进入调试模式（无参数或 b 为 true）或退出调试模式（b 为 false）。 debug.pause () 在调试模式下暂停执行。 debug.cont () 在调试模式下继续执行。 debug.step () 在调试模式下逐步执行。 debug.stepi () 在调试模式下逐步执行进入。 debug.stepo () 在调试模式下逐步执行退出。 debug.watch ([name])
...
scene.setenv({camtype='ortho', bgcolor='black'}) scene.addobj (type [, opttab]) 向场景的（0, 0, 0）位置添加具有特定类型和选项的对象。返回此对象。可用的类型包括： "points"：3D 离散点。使用这些选项：顶点、可选择性、颜色、不透明度、高亮颜色、大小。
...
".geojson"：GeoJSON 文件。使用这些选项：可选择性、颜色、不透明度、高亮颜色。 opttab 中的选项列表： vertices: 一个数组，其中每三个数字代表一个三维点（x，y，z）。对于网格，每三个点组成一个三角形。法线遵循右手定则。默认为空。
...
text: 标签使用。默认为“”。
font: 标签使用。默认为“Arial”。 这里是一些向场景添加对象的示例：
...
scene.render() --渲染场景 scene.getobj (name) 在场景中搜索，并返回第一个匹配名称的对象。相机和被选中的对象可以用"camera"和"selected"找到。 scene.tovector (radius, phi, theta) 从球坐标的转换返回 x、y、z。 scene.topolar (x, y, z) 从矢量返回球坐标：半径、phi、theta。 scene.render ([ms]) 渲染场景并检查浏览器，类似 os.getready，并返回相同结果。可以通过毫秒参数控制渲染之间的时间间隔。`},{header:"Object",slug:"object",content:`这部分函数控制场景中的对象。请注意，调用 obj:func(args) 是 scene.func(obj, args) 的语法糖。 obj:getpos () 返回对象的位置：x、y、z。 obj:setpos (x, y, z) 设置对象的位置：x、y、z。 obj:getrot () 返回对象的旋转：x、y、z。 obj:setrot (x, y, z) 设置对象的旋转：x、y、z。 obj:getscale () 返回对象的缩放：x、y、z。 obj:setscale (x, y, z) 设置对象的缩放：x、y、z。 obj:getchildren () 获取所有子对象并将它们放入作为返回值的表中。 obj:setchildren (childtab) 将表 childtab 中的所有对象设置为对象的子对象。 obj:getparent () 返回对象的父级。 obj:setparent (parent) 设置对象的父级。 obj:getmat () 返回物体的材质信息表。信息列表: 类型: 一个材质类型字符串，比如"MeshBasicMaterial"。
贴图: 一个贴图编号。
颜色: 一个颜色字符串，如"red"或"#FF0000"。
透明度: 一个表示材质多透明的数字。 obj:setmat (opttab) 从opttab中的选项设置物体的材质。选项列表: 类型: 一个材质类型字符串，比如"MeshBasicMaterial"。
贴图: 一个贴图编号或者一个图片文件路径或URL，比如"map.jpg"。
颜色: 一个颜色字符串，如"red"或"#FF0000"。
透明度: 一个表示材质多透明的数字。 obj:getvertices () 返回物体几何体的顶点数组和索引数组（如果有）。 obj:setvertices (vertices [, indices]) 设置物体几何的顶点（数组）和索引（数组，如果有）。 obj:getdata () 返回存储物体的自定义数据的表。示例代码请参考2.2搜索国家。 obj:setdata (datatab) 将datatab中的（键，值）写入物体的自定义数据字段。 obj:tojson (fpath) 将对象写入到指定路径的Threejs JSON文件中。
obj:delete()
递归删除对象及其所有子对象。 本文使用ChatGPT翻译，如有遗漏请反馈。`}]},{path:"/docs/web/4.4_discrete_event_simulation.html",title:"4.4 离散事件仿真",pathLocale:"/",contents:[{header:"4.4 离散事件仿真",slug:"_4-4-离散事件仿真",content:"在仿真中，事件需要随机生成并安排时间，因此Lua中的随机数和协程功能已经得到改进。"},{header:"随机数生成",slug:"随机数生成",content:`随机数的概率分布已得到扩展，但传统用法仍然保留。 math.randomseed (x, [, disttab]) 根据整数参数x和可选的disttab表返回一个种子。 disttab中的选项包括： dist：概率分布，可选值为"normal"、"exponential"、"poisson"和"uniform"。默认为"uniform"。
mu：均值或期望值，默认为1。
...
for i = 1, 10 do print(seed:random())
...
从空间数据到物流模型`},{header:"函数索引",slug:"函数索引",content:`协程:
