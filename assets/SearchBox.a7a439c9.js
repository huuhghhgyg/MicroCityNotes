import{g as b,h as C,i as D,j as A,t as R,u as I,k as N,_ as E,o as u,c as p,l as _,v as O,a as m,n as S,m as y,F as M,p as x,e as G,q as w,s as F}from"./app.d2d20754.js";const z=[{path:"/",title:"MicroCity笔记",pathLocale:"/",contents:[{header:"MicroCity笔记",slug:"microcity笔记",content:`目前正在施工中🧱...
MicroCity文档：提供全文搜索功能及部分解释
笔记：学习笔记
...
Update(Nodes)
Update(Links) 示例改自MicroCity自带的编辑器 虽然一开始就输入了网络连线，但是在拓补化的过程中可能由于连线上的节点重合，在拓补化的过程中会把连续的连线拆成多段，因此得到的Links图层可能与输入的图层存在不同。而拓补化得到的节点Nodes和得到的连线Links是对应的，因此推荐使用拓补化处理完的Links图层作为网络节点之间的连线。`},{header:"CopyShapeTo函数",slug:"copyshapeto函数",content:`拓补的操作中使用了CopyShapeTo()函数，其作用是将一个Shape对象复制到Shapes图层中的对应位置，并根据输入的dx和dy在Shapes图层中进行位置变换。其中，dx和dy分别为在x和y方向上的位移量（理解为deltax和deltay）。
CopyShapeTo(TargetShapes, dx, dy, Shape1 [, Shape2, ...])
根据函数的用法可以看到，还支持复制到多个图层上的对应位置后再进行变换。
如果只想将图形复制到对应图层的对应位置，而不进行变换，可以将dx和dy设为0，像这样：
CopyShapeTo(TargetShapes, 0, 0, Shape1 [, Shape2, ...])
CopyShapeTo()函数的具体用法见4.3 矢量图形和表格`}]},{path:"/notes/visualization.html",title:"结果可视化",pathLocale:"/",contents:[{header:"结果可视化",slug:"结果可视化",content:""},{header:"改变形状颜色",slug:"改变形状颜色",content:`在程序界面左下角的图形属性框内可以通过改变Colors栏目下的Type属性改变显示颜色的种类。共有3种选项： Unique Symbol：指定颜色
...
标签尺寸参考系设置：Labels栏目下的Size relates to...调整标签尺寸的参考系，Default Size调整的是标签(Labels)相对于坐标系的尺寸。`},{header:"更改显示的标签",slug:"更改显示的标签",content:"在程序界面左下角的图形属性框内可以设置图形及其标签的尺寸参考系。上图中显示的标签为ID，如果需要改为表格中的其他列，可以更改Lables下的Attribute属性实现。"}]},{path:"/docs/imgs/",title:"",pathLocale:"/",contents:[{header:"",slug:"",content:"This folder contains images used by documentation pages."}]},{path:"/404.html",title:"",pathLocale:"/",contents:[]}],B="update-vuepress-plugin-full-text-search2-search-index";var P=b(z),U=C(()=>{const e=new Map;for(const t of P.value)e.set(t.path,t);return e});import.meta.webpackHot&&(__VUE_HMR_RUNTIME__[B]=e=>{P.value=e});function Y(e){const t=b([]);let r=null;return D(e,()=>{r&&clearTimeout(r),r=setTimeout(o,100)}),t;function o(){const c=e.value.toLowerCase().trim();if(!c){t.value=[];return}const i=new Map,l=new Set;for(const a of P.value)for(const s of V(a,c)){l.add(s.parentPageTitle);let d=i.get(s.parentPageTitle);d||(d=[],i.set(s.parentPageTitle,d)),d.push(s)}const n=[...l].sort((a,s)=>{const d=i.get(a);return i.get(s).length-d.length});t.value=[...i].flatMap(([,a])=>a).sort((a,s)=>a.parentPagePriority-s.parentPagePriority||n.indexOf(a.parentPageTitle)-n.indexOf(s.parentPageTitle)||a.priority-s.priority)}}function*V(e,t){const r=v(e.title,t);if(r){yield{path:e.path,parentPageTitle:T(e),title:e.title,display:r,page:e,content:null,parentPagePriority:1,priority:1};return}for(const o of e.contents){const c=v(o.header,t);if(c){yield{path:e.path+(o.slug?`#${o.slug}`:""),parentPageTitle:T(e),title:e.title,display:c,page:e,content:null,parentPagePriority:10,priority:2};continue}const i=v(o.content,t);i&&(yield{path:e.path+(o.slug?`#${o.slug}`:""),parentPageTitle:T(e),title:e.title,display:[{type:"header",str:`${o.header}
`},...i],page:e,content:null,parentPagePriority:10,priority:10})}}function T(e){const t=e.path.split("/");let r="/";return t[1]&&(r=`/${t[1]}/`),(U.value.get(r)||e).title}function v(e,t){const r=[];let o=0;const c=e.toLowerCase().replace(/\s/gu," ");let i=0,l=c.indexOf(t,i);if(l<0)return null;for(;l>=0;){const a=l+t.length;if(n(e.slice(i,l),"normal"),n(e.slice(l,a),"highlight"),i=a,l=c.indexOf(t,i),o>100)break}return n(e.slice(i),"normal"),r.filter(a=>a.str);function n(a,s){let d=a;s==="normal"&&d.length>100&&o===0&&(d=`… ${d.slice(-10)}`);let f=!1;if(o+d.length>100){if(r.some(g=>g.type==="ellipsis"))return;d=d.slice(0,Math.max(100-o,1)),f=!0}r.push({type:s,str:d}),o+=d.length,f&&(r.push({type:"ellipsis",str:" …"}),o+=2)}}const X={"/":{placeholder:"搜索"}},W=A({name:"SearchBox",props:{locales:{type:Object,required:!1,default:()=>X}},setup(e){const{locales:t}=R(e),r=b(""),o=b(!1),c=b(-1),i=Y(r),l=C(()=>r.value&&o.value&&i.value.length),n=I(),a=N(),s=C(()=>{var h;return(h=t.value[a.value])!=null?h:{}});function d(){if(!l.value)return;let h=c.value-1;h<0&&(h=i.value.length-1),g(h)}function f(){if(!l.value)return;let h=c.value+1;h>=i.value.length&&(h=0),g(h)}function g(h){c.value=h}function L(){c.value=-1}function j(h){if(!l.value)return;const k=i.value[h];k&&n.push(k.path)}return{query:r,focused:o,focusIndex:c,suggestions:i,activeSuggestion:l,onUp:d,onDown:f,focus:g,unfocus:L,go:j,locale:s}}});const q={class:"search-box",role:"search"},$=["placeholder"],H=["onMousedown","onMouseenter"],Z=["href"],Q={key:0,class:"parent-page-title"},J={class:"suggestion-row"},K={class:"page-title"},ee={class:"suggestion-content"};function te(e,t,r,o,c,i){var l;return u(),p("div",q,[_(m("input",{ref:"input","onUpdate:modelValue":t[0]||(t[0]=n=>e.query=n),"aria-label":"Search",class:S({focused:e.focused}),placeholder:(l=e.locale.placeholder)!=null?l:"Search",autocomplete:"off",spellcheck:"false",onFocus:t[1]||(t[1]=()=>e.focused=!0),onBlur:t[2]||(t[2]=()=>e.focused=!1),onKeyup:[t[3]||(t[3]=y(n=>e.go(e.focusIndex),["enter"])),t[4]||(t[4]=y((...n)=>e.onUp&&e.onUp(...n),["up"])),t[5]||(t[5]=y((...n)=>e.onDown&&e.onDown(...n),["down"]))]},null,42,$),[[O,e.query]]),e.activeSuggestion?(u(),p("ul",{key:0,class:"suggestions",onMouseleave:t[7]||(t[7]=(...n)=>e.unfocus&&e.unfocus(...n))},[(u(!0),p(M,null,x(e.suggestions,(n,a)=>(u(),p("li",{key:a,class:S(["suggestion",{focused:a===e.focusIndex}]),onMousedown:s=>e.go(a),onMouseenter:s=>e.focus(a)},[m("a",{href:n.path,onClick:t[6]||(t[6]=F(()=>{},["prevent"]))},[n.parentPageTitle&&(!e.suggestions[a-1]||e.suggestions[a-1].parentPageTitle!==n.parentPageTitle)?(u(),p("div",Q,w(n.parentPageTitle),1)):G("v-if",!0),m("div",J,[m("div",K,w(n.title||n.path),1),m("div",ee,[(u(!0),p(M,null,x(n.display,(s,d)=>(u(),p("span",{key:d,class:S(s.type)},w(s.str),3))),128))])])],8,Z)],42,H))),128))],32)):G("v-if",!0)])}const ae=E(W,[["render",te],["__scopeId","data-v-fd6cd4d5"],["__file","SearchBox.vue"]]);export{ae as default};
