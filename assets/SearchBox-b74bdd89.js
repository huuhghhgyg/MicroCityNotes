...
obj:delete()`}]},{path:"/notes/3d-objects.html",title:"3D 对象",pathLocale:"/",contents:[{header:"3D 对象",slug:"_3d-对象",content:"本节将介绍 MicroCity Web 中的三维场景对象及其控制函数。"},{header:"添加对象",slug:"添加对象",content:`向场景中添加对象，函数形式如下
...
3D 对象 [EN]
离散事件模拟和程序控制 [EN]
混合整数规划 [EN]`},{header:"思路",slug:"思路",content:`仓库仿真
通用绘图代码
...
-- 1 10 3 4 5 PrintList(list1)
-- 1 2 3 4 5 (没有发生变化)
在这个例子中，修改深拷贝得到的list2中的元素并不会对原来的list1产生影响。`}]},{path:"/notes/mip-en.html",title:"Mixed integer programming",pathLocale:"/",contents:[{header:"Mixed integer programming",slug:"mixed-integer-programming",content:`Mixed-Integer Programming (MIP) is a powerful mathematical optimization technique used to solve problems where some of the decision variables are required to be integers. This article provides a step-by-step guide on creating and solving MIP models using Lua programming language in the context of MicroCity Web. Translate info
...
Afterward, you can proceed with the general process 😎`}]},{path:"/notes/mip.html",title:"混合整数规划",pathLocale:"/",contents:[{header:"混合整数规划",slug:"混合整数规划",content:"本文档介绍MicroCity Web中混合整数规划的建模方法。"},{header:"创建混合整数规划模型",slug:"创建混合整数规划模型",content:`创建整数规划模型对象
...
Three.js Editor Three.js的在线3D对象编辑器。 🔨 Work In Progress...`}]},{path:"/notes/versions.html",title:"MicroCity的版本",pathLocale:"/",contents:[{header:"MicroCity的版本",slug:"microcity的版本",content:""},{header:"MicroCity",slug:"microcity",content:"MicroCity软件最初推出时只有一个桌面版本，只能在Windows操作系统上运行。随着时间的推移，开发者（@mixwind)意识到这个限制使得许多潜在的用户无法使用该软件，因为他们的电脑使用的可能是其他操作系统，比如MacOS、Linux等。"},{header:"MicroCityWeb",slug:"microcityweb",content:`🎯访问MicroCity Web 📍(离线版)下载
...
但是由于Chromium浏览器内核的更新，某些老旧的系统版本可能不受支持（如Windows 8.1及以下的Windows版本、32位的Windows操作系统等）`},{header:"兼容性",slug:"兼容性",content:`由于使用了实验性的javascript函数，MicroCityWeb可能在以下浏览器内核中运行存在兼容性问题： iOS和iPad OS上的大多数浏览器（由于Apple强制使用Safari的浏览器内核）
FireFox浏览器 2023/9/29测试遇到的兼容性问题及部分技术细节 iOS和iPad OS： 无法打开本地文件（由于使用了showOpenFilePicker()方法） FireFox： 无法打开本地文件（由于使用了showOpenFilePicker()方法） 关于showOpenFilePicker()方法的影响参见 MDN Web Docs | showOpenFilePicker()`},{header:"总结",slug:"总结",content:`MicroCityWeb版的推出使得软件更加易用和普及，让更多的用户可以从中受益。同时，MicroCity桌面版本仍然可用，使得那些已经习惯于使用该版本的用户可以继续使用，并让他们更容易地过渡到MicroCityWeb版。
...
MicroCityWeb版本号: 表示当前使用的MicroCityWeb版本号。如果与已知版本有差异，可以通过强制刷新加载最新版本。`},{header:"注意事项",slug:"注意事项",content:"关闭自动翻译： 如果你的操作系统语言不是英语且浏览器开启了自动翻译，需要手动关闭，否则代码运行将出现错误。"}]},{path:"/docs/imgs/",title:"",pathLocale:"/",contents:[{header:"",slug:"",content:"This folder contains images used by documentation pages."}]},{path:"/404.html",title:"",pathLocale:"/",contents:[]}],W="update-vuepress-plugin-full-text-search2-search-index";var C=f(V),F=w(()=>{const e=new Map;for(const t of C.value)e.set(t.path,t);return e});import.meta.webpackHot&&(__VUE_HMR_RUNTIME__[W]=e=>{C.value=e});function B(e){const t=f([]);let s=null;return A(e,()=>{s&&clearTimeout(s),s=setTimeout(i,100)}),t;function i(){const d=e.value.toLowerCase().trim();if(!d){t.value=[];return}const a=new Map,n=new Set;for(const o of C.value)for(const r of q(o,d)){n.add(r.parentPageTitle);let l=a.get(r.parentPageTitle);l||(l=[],a.set(r.parentPageTitle,l)),l.push(r)}const c=[...n].sort((o,r)=>{const l=a.get(o);return a.get(r).length-l.length});t.value=[...a].flatMap(([,o])=>o).sort((o,r)=>o.parentPagePriority-r.parentPagePriority||c.indexOf(o.parentPageTitle)-c.indexOf(r.parentPageTitle)||o.priority-r.priority)}}function*q(e,t){const s=v(e.title,t);if(s){yield{path:e.path,parentPageTitle:j(e),title:e.title,display:s,page:e,content:null,parentPagePriority:1,priority:1};return}for(const i of e.contents){const d=v(i.header,t);if(d){yield{path:e.path+(i.slug?`#${i.slug}`:""),parentPageTitle:j(e),title:e.title,display:d,page:e,content:null,parentPagePriority:10,priority:2};continue}const a=v(i.content,t);a&&(yield{path:e.path+(i.slug?`#${i.slug}`:""),parentPageTitle:j(e),title:e.title,display:[{type:"header",str:`${i.header}
`},...a],page:e,content:null,parentPagePriority:10,priority:10})}}function j(e){const t=e.path.split("/");let s="/";return t[1]&&(s=`/${t[1]}/`),(F.value.get(s)||e).title}function v(e,t){const s=[];let i=0;const d=e.toLowerCase().replace(/\s/gu," ");let a=0,n=d.indexOf(t,a);if(n<0)return null;for(;n>=0;){const o=n+t.length;if(c(e.slice(a,n),"normal"),c(e.slice(n,o),"highlight"),a=o,n=d.indexOf(t,a),i>100)break}return c(e.slice(a),"normal"),s.filter(o=>o.str);function c(o,r){let l=o;r==="normal"&&l.length>100&&i===0&&(l=`… ${l.slice(-10)}`);let g=!1;if(i+l.length>100){if(s.some(b=>b.type==="ellipsis"))return;l=l.slice(0,Math.max(100-i,1)),g=!0}s.push({type:r,str:l}),i+=l.length,g&&(s.push({type:"ellipsis",str:" …"}),i+=2)}}const U={"/":{placeholder:"搜索"}},Y=L({name:"SearchBox",props:{locales:{type:Object,required:!1,default:()=>U}},setup(e){const{locales:t}=D(e),s=f(""),i=f(!1),d=f(-1),a=B(s),n=w(()=>s.value&&i.value&&a.value.length),c=I(),o=z(),r=w(()=>t.value[o.value]??{});function l(){if(!n.value)return;let h=d.value-1;h<0&&(h=a.value.length-1),b(h)}function g(){if(!n.value)return;let h=d.value+1;h>=a.value.length&&(h=0),b(h)}function b(h){d.value=h}function P(){d.value=-1}function G(h){if(!n.value)return;const M=a.value[h];M&&c.push(M.path)}return{query:s,focused:i,focusIndex:d,suggestions:a,activeSuggestion:n,onUp:l,onDown:g,focus:b,unfocus:P,go:G,locale:r}}});const H={class:"search-box",role:"search"},X=["placeholder"],Z=["onMousedown","onMouseenter"],$=["href"],K={key:0,class:"parent-page-title"},Q={class:"suggestion-row"},J={class:"page-title"},ee={class:"suggestion-content"};function te(e,t,s,i,d,a){return u(),p("div",H,[N(m("input",{ref:"input","onUpdate:modelValue":t[0]||(t[0]=n=>e.query=n),"aria-label":"Search",class:x({focused:e.focused}),placeholder:e.locale.placeholder??"Search",autocomplete:"off",spellcheck:"false",onFocus:t[1]||(t[1]=()=>e.focused=!0),onBlur:t[2]||(t[2]=()=>e.focused=!1),onKeyup:[t[3]||(t[3]=y(n=>e.go(e.focusIndex),["enter"])),t[4]||(t[4]=y((...n)=>e.onUp&&e.onUp(...n),["up"])),t[5]||(t[5]=y((...n)=>e.onDown&&e.onDown(...n),["down"]))]},null,42,X),[[O,e.query]]),e.activeSuggestion?(u(),p("ul",{key:0,class:"suggestions",onMouseleave:t[7]||(t[7]=(...n)=>e.unfocus&&e.unfocus(...n))},[(u(!0),p(k,null,T(e.suggestions,(n,c)=>(u(),p("li",{key:c,class:x(["suggestion",{focused:c===e.focusIndex}]),onMousedown:o=>e.go(c),onMouseenter:o=>e.focus(c)},[m("a",{href:n.path,onClick:t[6]||(t[6]=E(()=>{},["prevent"]))},[n.parentPageTitle&&(!e.suggestions[c-1]||e.suggestions[c-1].parentPageTitle!==n.parentPageTitle)?(u(),p("div",K,S(n.parentPageTitle),1)):_("v-if",!0),m("div",Q,[m("div",J,S(n.title||n.path),1),m("div",ee,[(u(!0),p(k,null,T(n.display,(o,r)=>(u(),p("span",{key:r,class:x(o.type)},S(o.str),3))),128))])])],8,$)],42,Z))),128))],32)):_("v-if",!0)])}const oe=R(Y,[["render",te],["__scopeId","data-v-fd6cd4d5"],["__file","SearchBox.vue"]]);export{oe as default};
