...
::: tip
如果需要套用现成模型，或者申请上传模型到本文作者维护的仓库，请参考文档 [3D对象-外部模型](./3d_objects.md#外部模型)
:::
...
* **对于6个货架的情况**，选取了3种情况进行多次仿真，得到每种情况出入库耗费Ticks的平均值、最大值、最小值。
  > 当前Ticks是AGV移动的次数，相当于单位操作时间。后面对这种情况进行改进
...
eventcond(no,bottom)->sumtime
```
这样就改为了以时间为核心的仿真。由于仿真以时间为核心，因此就需要设定各种操作的速率/时间，保证时间计算的准确性。在MicroCityWeb中，有一些技巧可以用于时间推进法的仿真。
